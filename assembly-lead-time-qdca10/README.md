...
[OrderEvents](../order-events/README.md) の `LINE_ITEM_STATUS_CHANGED` イベント(`assemblyLine = QDCA10`)から、明細(itemId)単位の `PLACED → IN_PROGRESS → FULFILLED` のリードタイムを算出するデータプロダクトである。
...
[Assembly Lead Time QDCA10pro](../assembly-lead-time-qdca10pro/README.md) と同一スキーマで出力しており、Trino 側で `UNION ALL` すれば機種間比較が可能。
