...
[OrderEvents](../order-events/README.md) の `LINE_ITEM_STATUS_CHANGED` イベント(`assemblyLine = QDCA10PRO`)から、明細(itemId)単位の `PLACED → IN_PROGRESS → FULFILLED` のリードタイムを算出するデータプロダクトである。[Assembly Lead Time QDCA10](../assembly-lead-time-qdca10/README.md) と同一スキーマ・同一パターンで、対象を QDCA10pro に絞ったもの。
